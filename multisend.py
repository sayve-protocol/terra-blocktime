...
for x in range(0,len(terra_address)):
        ...
        # sleep(6)
        sleep(6)
tx = wallet.create_and_sign_tx(msgs=msgall, memo="testing all transfer onetime")
result = terra.tx.broadcast(tx)
print(result)

for line in range(0,len(terra_address)):
    data = []
    data.append(terra_address[line])
    data.append(sayve_amount[line])
    data.append(ust_amount[line])
    data.append(result.txhash)
    all.append(data)
